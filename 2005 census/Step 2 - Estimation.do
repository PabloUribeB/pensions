...
global tables "C:\Users\Pablo Uribe\Desktop\BanRep\Pensions\Tables"
global graphs "C:\Users\Pablo Uribe\Desktop\BanRep\Pensions\Graphs"
...
global covars sex ill mdcar mdorg mdneu mdtra mdcon mdjoi mddia mdbur mdaid mdche mdint lit empstat pension atleast_highschool wall garbage elec sewer runwat natgas phone toilet water kitchex watsrc refrig washer stereo watheat shower blender oven aircond fan tvc compute microw  more4peop_dwelling cement_floor atleast1bathrm owns_dwelling fuel_cook has_moto has_bike enough_income has_car
...
matrix rownames balance= $rownames
...
coefplot matrix(balance[,1]), ci((balance[,2] balance[,3])) bylabels(, wrap(15)) byopts(compact rows(1) legend(off)) scheme(s1color) graphregion(fcolor(white))  plotregion(fcolor(white))  msize(medium) xline(0, lwidth(thin)) ciopts(recast(rcap) lcolor(black)) ylabel(, labsize(*0.5)) mfcolor(white)  subtitle(, size(small) color(black) bcolor(white) justification(center)) grid(between glpattern(dash) glwidth(vvthin) glcolor(black))  msymbol(o) mlcolor(black)  mlabposition(0) coeflabels(,wrap(45)) groups("Was ill in past year"-"Intensive care" = "{bf:Health}" ///
		 "Literate"-"Completed at least high school" = "{bf:Personal}" ///
		 "Cement, brick, stone wall"-"Income enough for basic expenses" = "{bf:Household}", labs(*0.6)) note(Joint test p-value: `pvalue')
...
matrix rownames balanceRD= $rownames

matsave balanceRD, replace saving path("$tables")

coefplot matrix(balanceRD[,1]), ci((balanceRD[,2] balanceRD[,3])) bylabels(, wrap(15)) byopts(compact rows(1) legend(off)) scheme(s1color) graphregion(fcolor(white))  plotregion(fcolor(white))  msize(medium) xline(0, lwidth(thin)) ciopts(recast(rcap) lcolor(black)) ylabel(, labsize(*0.5)) mfcolor(white)  subtitle(, size(small) color(black) bcolor(white) justification(center)) grid(between glpattern(dash) glwidth(vvthin) glcolor(black))  msymbol(o) mlcolor(black)  mlabposition(0) coeflabels(,wrap(45)) groups("Was ill in past year"-"Intensive care" = "{bf:Health}" ///
		 "Literate"-"Completed at least high school" = "{bf:Personal}" ///
...
**# Final covariates used
gen above = (std_weeks >= 0)
gen std_weeks2 = std_weeks^2

global covars ill mdcar mdorg mdneu mdtra mdcon mdjoi mddia mdbur mdaid mdche mdint empstat pension atleast_highschool
...
matrix rownames balanceRD_final= $rownames

matsave balanceRD_final, replace saving path("$tables")

coefplot matrix(balanceRD_final[,1]), ci((balanceRD_final[,2] balanceRD_final[,3])) bylabels(, wrap(15)) byopts(compact rows(1) legend(off)) scheme(s1color) graphregion(fcolor(white))  plotregion(fcolor(white))  msize(medium) xline(0, lwidth(thin)) ciopts(recast(rcap) lcolor(black)) ylabel(, labsize(*0.5)) mfcolor(white)  subtitle(, size(small) color(black) bcolor(white) justification(center)) grid(between glpattern(dash) glwidth(vvthin) glcolor(black))  msymbol(o) mlcolor(black)  mlabposition(0) coeflabels(,wrap(45)) groups("Was ill in past year"-"Intensive care" = "{bf:Health}" ///
...
matrix rownames balanceRD_final_M50= $rownames

matsave balanceRD_final_M50, replace saving path("$tables")

coefplot matrix(balanceRD_final_M50[,1]), ci((balanceRD_final_M50[,2] balanceRD_final_M50[,3])) bylabels(, wrap(15)) byopts(compact rows(1) legend(off)) scheme(s1color) graphregion(fcolor(white))  plotregion(fcolor(white))  msize(medium) xline(0, lwidth(thin)) ciopts(recast(rcap) lcolor(black)) ylabel(, labsize(*0.5)) mfcolor(white)  subtitle(, size(small) color(black) bcolor(white) justification(center)) grid(between glpattern(dash) glwidth(vvthin) glcolor(black))  msymbol(o) mlcolor(black)  mlabposition(0) coeflabels(,wrap(45)) groups("Was ill in past year"-"Intensive care" = "{bf:Health}" ///
		 "Works"-"Completed at least high school" = "{bf:Personal}", labs(*0.6))
...
matrix rownames balanceRD_final_F55= $rownames

matsave balanceRD_final_F55, replace saving path("$tables")

coefplot matrix(balanceRD_final_F55[,1]), ci((balanceRD_final_F55[,2] balanceRD_final_F55[,3])) bylabels(, wrap(15)) byopts(compact rows(1) legend(off)) scheme(s1color) graphregion(fcolor(white))  plotregion(fcolor(white))  msize(medium) xline(0, lwidth(thin)) ciopts(recast(rcap) lcolor(black)) ylabel(, labsize(*0.5)) mfcolor(white)  subtitle(, size(small) color(black) bcolor(white) justification(center)) grid(between glpattern(dash) glwidth(vvthin) glcolor(black))  msymbol(o) mlcolor(black)  mlabposition(0) coeflabels(,wrap(45)) groups("Was ill in past year"-"Intensive care" = "{bf:Health}" ///
		 "Works"-"Completed at least high school" = "{bf:Personal}", labs(*0.6))
